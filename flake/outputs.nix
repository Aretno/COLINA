...
, ...
  ...
  in {
    ...
    packages.default = self.packages.COLINA;

    # Executed by `nix run <URL of this flake>#COLINA -- <args?>`
    # TODO: switch to clang environment
    packages.COLINA = pkgs.stdenv.mkDerivation {
      pname = "COLINA";
      version = "0.0.0";
      src = "${self}/src";
      nativeBuildInputs = [];
    ...
    apps.default = self.apps.COLINA;

    # Executed by `nix run <URL of this flake>#COLINA`
    apps.COLINA = let
      g4-data = nain4.deps.g4-data-package;
      COLINA-app-package = pkgs.writeShellScriptBin "COLINA" ''
        export PATH=${pkgs.lib.makeBinPath [ self.packages.COLINA ]}:$PATH
        ...
        exec COLINA --macro-path ${self}/macs "$@"
      '';
    in { type = "app"; program = "${COLINA-app-package}/bin/COLINA"; };
    ...
    packages.singularity = pkgs.singularity-tools.buildImage {
      name = "COLINA";
      contents = [ self.apps.COLINA.program ];
      runScript = "${self.apps.COLINA.program} $@";
      diskSize = 10240;
      memSize = 5120;
    };
